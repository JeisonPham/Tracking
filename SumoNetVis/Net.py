...
class _Edge:
    ...
    def polygon(self, ax, lane_kwargs=None, lane_marking_kwargs=None, **kwargs):
        """
        Plots the lane.
        The lane_kwargs and lane_markings_kwargs override the general kwargs for their respective functions.
        ...
        if lane_marking_kwargs is None:
            lane_marking_kwargs = dict()
        for lane in self.lanes:
            lane_artist = lane.polygon_shape(ax, **{**kwargs, **lane_kwargs})
            lane_marking_artist = lane.polygon_lane_markings(ax, **{**kwargs, **lane_marking_kwargs})
            lane_artists.append(lane_artist)
            lane_marking_artists += lane_marking_artist
        return lane_artists, lane_marking_artists
...
class _LaneMarking:
    ...
    def polygon(self, ax, **kwargs):
        color = kwargs.pop("color") if "color" in kwargs else self.color
        x, y = zip(*self.alignment.coords)
        line = self.alignment.buffer(self.linewidth / 2, cap_style=CAP_STYLE.flat)
        interp = interp1d(x=[0.0, 1.], y=[x, y], kind="linear")
        # for i in range(1, 10):
        #     start = (i - 1) / 10
        #     end = i / 10
        #     start_pos = interp(start)
        #     end_pos = interp(end)
        #
        #     adjusted_start = 0.75 * start_pos + 0.25 * end_pos
        #     adjusted_end = 0.25 * start_pos + 0.75 * end_pos
        #     line = LineString([adjusted_start, adjusted_end])
        #     line = line.buffer(self.linewidth/2, cap_style=CAP_STYLE.flat)
    ...
    def get_as_3d_object(self, z=0.001, extrude_height=0, include_bottom_face=False):
        ...
        return _Utils.Object3D.from_shape(oriented_shape, self.purpose + "_marking", self.color + "_marking", z=z,
                                          extrude_height=extrude_height, include_bottom_face=include_bottom_face)
...
class _Lane:
    ...
    def polygon_shape(self, ax, **kwargs):
        """
        Plots the entire shape of the lane
    ...
    def get_markings_as_3d_objects(self, z_lane=0, extrude_height=0, include_bottom_face=False):
        ...
        for marking in self._guess_lane_markings():
            ...
            try:
                obj = marking.get_as_3d_object(z=z, extrude_height=extrude_height,
                                               include_bottom_face=include_bottom_face)
    ...
    def get_as_3d_object(self, z=0, include_bottom_face=False, material_param=None, extrude_height_param=None,
        ...
        return _Utils.Object3D.from_shape(orient(self.shape), self.id, material, z=z, extrude_height=h,
                                          include_bottom_face=include_bottom_face)
    ...
    def polygon_lane_markings(self, ax, **kwargs):
        """
        Guesses and plots some simple lane markings.
...
class _Connection:
    ...
    def _generate_shape(self):
        ...
        # Get lane edges
        from_lane_left_edge = [list(c) for c in
                               self.from_lane.alignment.parallel_offset(self.from_lane.width / 2, side="left").coords]
        from_lane_right_edge = [list(c) for c in
                                self.from_lane.alignment.parallel_offset(self.from_lane.width / 2, side="right").coords]
        to_lane_left_edge = [list(c) for c in
                             self.to_lane.alignment.parallel_offset(self.to_lane.width / 2, side="left").coords]
        to_lane_right_edge = [list(c) for c in
                              self.to_lane.alignment.parallel_offset(self.to_lane.width / 2, side="right").coords]
        try:
            left_edge = [list(c) for c in
                         self.via_lane.alignment.parallel_offset(self.from_lane.width / 2, side="left").coords]
        except (ValueError, NotImplementedError):
            left_edge = []  # if offset fails, don't use any intermediate coordinates
        try:
            right_edge = [list(c) for c in
                          self.via_lane.alignment.parallel_offset(self.from_lane.width / 2, side="right").coords]
        except (ValueError, NotImplementedError):
    ...
    def get_as_3d_object(self, z=0, include_bottom_face=False):
        ...
        return _Utils.Object3D.from_shape(orient(shape), "cxn_via_" + self.via_id, material, z=z, extrude_height=h,
                                          include_bottom_face=include_bottom_face)
...
class _Junction:
    ...
    def get_as_3d_object(self, z=0, extrude_height=0, include_bottom_face=False, material_param=None,
        ...
        return _Utils.Object3D.from_shape(orient(self.shape), self.id, "junction", z=z, extrude_height=extrude_height,
                                          include_bottom_face=include_bottom_face)
...
class Net:
    ...
    def generate_obj_text(self, style=None, stripe_width_scale=1, terrain_distance=0, terrain_z=0, terrain_hi_q=False,
        ...
        for object in objects:
            temp = np.asarray(object.vertices)
            temp[:, (1, 2)] = temp[:, (2, 1)]
            object.vertices = temp.tolist()
    ...
    def plot(self, ax=None, clip_to_limits=False, zoom_to_extents=True, style=None, stripe_width_scale=1,
        ...
        for edge in self.edges.values():
            if edge.function != "internal" and (not clip_to_limits or edge.intersects(window)):
                la, lma = edge.plot(ax, {"zorder": -100, **lane_kwargs}, {"zorder": -90, **lane_marking_kwargs},
                                    **kwargs)
    ...
    def generate_polygons(self, style=None, stripe_width_scale=1, terrain_distance=0, terrain_z=0, terrain_hi_q=False,
                          material_mapping=None, material_param=None, extrude_height_param=None,
                          extrude_height_param_transform=None, apply_netOffset=False):
        """
        Generates the contents for a Wavefront-OBJ file which represents the network as a 3D model.
        ...
        if style is not None:
            set_style(style)
        set_stripe_width_scale(stripe_width_scale)
        objects = {"junction": {}, "connections": [], "additional": {},
                   "lane": {}, "lane_markings": {}}
        for edge in self.edges.values():
            if edge.function == "internal":
                continue
            for lane in edge.lanes:
                if edge.function not in ["crossing", "walkingarea"]:
                    lane_obj = lane.get_as_3d_object(material_param=material_param,
                                                     extrude_height_param=extrude_height_param,
                                                     extrude_height_param_transform=extrude_height_param_transform)
                    objects["lane"][lane.id] = np.asarray(lane_obj.vertices)[:, :2].tolist()

                lane_markings = lane.get_markings_as_3d_objects()
                temp = []
                for marking in lane_markings:
                    temp.append(np.asarray(marking.vertices)[:, :2].tolist())
                objects["lane_markings"][lane.id] = temp

        for junction in self.junctions.values():
            if junction.shape is not None:
                junction_obj = junction.get_as_3d_object(material_param=material_param,
                                                         extrude_height_param=extrude_height_param,
                                                         extrude_height_param_transform=extrude_height_param_transform)
                objects["junction"][junction.id] = np.asarray(junction_obj.vertices)[:, :2].tolist()

        for connection in self.connections:
            if connection.via_id is not None:
                if connection.from_lane.lane_type() == "pedestrian" and connection.to_lane.lane_type() == "pedestrian":
                    connection_obj = connection.get_as_3d_object()
                    objects['connection'].append(np.asarray(connection_obj)[:, :2].tolist())
...
if __name__ == "__main__":
    net = Net(r"C:\Users\Jason\Sumo\2022-09-12-16-57-35\osm.net.xml")
    # with open("polygons.json", "w") as f:
    #     json.dump(net.generate_polygons(), f)
    with open("model.obj", 'w') as f:
        f.write(net.generate_obj_text())
    # import open3d as o3d
    # polygon = o3d.io.read_triangle_mesh("untitled.obj")
    # polygon.compute_vertex_normals()
    # o3d.visualization.draw_geometries([polygon])
